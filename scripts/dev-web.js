...
function main() {
  ...
  console.log('Iniciando backend (puerto 3000)...');
  const backend = spawnProc('node', ['servidor.js'], { cwd: backendDir });

  // Espera breve para que el backend arranque antes del frontend web
  setTimeout(() => {
    console.log('Iniciando frontend web (Expo) en puerto 8082...');
    const frontend = spawnProc('npm', ['run', 'web', '--', '--port', '8082'], { cwd: frontendDir });
